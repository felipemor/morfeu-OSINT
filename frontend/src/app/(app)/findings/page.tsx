...
export default function FindingsPage() {
  ...
  const deleteFinding = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`${SCANNER_URL}/findings/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete finding');
      return res.json();
    },
    onSuccess: () => { toast.success('Finding deleted'); queryClient.invalidateQueries({ queryKey: ['all-findings'] }); },
    onError: () => toast.error('Failed to delete finding'),
  });

  const addFinding = useMutation({
    mutationFn: async (f: NewFindingForm) => {
      const res = await fetch(`${SCANNER_URL}/findings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...f, cvss_score: parseFloat(f.cvss_score) || 0 }),
      });
                  ...
                  {expanded === f.id && (
                    <tr key={f.id + '-exp'}>
                      <td colSpan={8} className="p-0">
                        <div className="bg-bg-primary/70 border-t border-bg-border p-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
